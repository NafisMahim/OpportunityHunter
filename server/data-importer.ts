...
export class DataImporter {
  ...
  // New comprehensive file parsers for the latest batch
  async parseStructuredTXT(content: string): Promise<any[]> {
  ...
  async parseFreeSummerPrograms(content: string): Promise<any[]> {
  ...
  async parseOceanOpportunities(content: string): Promise<any[]> {
    const lines = content.split('\n');
    const opportunities = [];
    let currentTitle = '';
  ...
  private extractDescription(fields: string[]): string {
  ...
  private extractEligibility(fields: string[]): string {
    for (const field of fields) {
  ...
  private extractLocation(fields: string[]): string {
    for (const field of fields) {
  ...
  private extractCategories(fields: string[]): string {
  ...
  private extractMajor(title: string): string {
  ...
  private extractDeadline(fields: string[]): string {
    for (const field of fields) {
  ...
  private generatePlaceholderURL(title: string): string {
  ...
  // Import comprehensive files using the new parsing methods
  async importComprehensiveFiles(): Promise<void> {
    console.log('🚀 Starting comprehensive file import...');
    ...
    for (const fileName of files) {
      ...
      try {
        if (fileName.includes('Python-Data-Science') || fileName.includes('Computer-Science-Sum')) {
          opportunities = await this.parseStructuredTXT(content);
        } else if (fileName.includes('Free-Summer-Programs')) {
          opportunities = await this.parseFreeSummerPrograms(content);
        } else if (fileName.includes('OceanOpps')) {
          opportunities = await this.parseOceanOpportunities(content);
        }
        ...
        for (const opp of opportunities) {
          try {
            const cleanData: InsertOpportunity = {
              title: opp.title?.substring(0, 500) || '',
              description: opp.description?.substring(0, 2000) || '',
              organization: opp.categories?.substring(0, 500) || 'Educational Institution',
              type: this.determineOpportunityType(opp.title, opp.description, opp.major),
              source: `Comprehensive Import: ${fileName}`,
              url: opp.url?.substring(0, 1000) || '',
              deadline: opp.deadline?.substring(0, 500) || null,
              requirements: [opp.eligibility || 'High school students'],
              tags: [opp.major, opp.categories].filter(Boolean),
              relevancyScore: 85,
              amount: opp.cost !== 'Free' ? opp.cost : null,
              location: opp.location || 'Various',
              salary: null
            };

            await storage.createOpportunity(cleanData);
    ...
    console.log(`🎉 Total imported: ${totalImported} new opportunities`);
    console.log(`🔄 Duplicates skipped: ${totalDuplicates} opportunities`);
  }
}
