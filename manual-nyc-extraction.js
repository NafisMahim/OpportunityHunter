...
// Manual extraction of clear NYC opportunities from the Stuyvesant bulletins
...
async function saveNYCOpportunities() {
  try {
    await fs.writeFile('nyc-opportunities-manual.json', JSON.stringify(nycOpportunities, null, 2));
    console.log(`Saved ${nycOpportunities.length} manually extracted NYC opportunities to nyc-opportunities-manual.json`);
    
    // Print summary
    console.log('\nOpportunities by type:');
    const typeCount = {};
    nycOpportunities.forEach(opp => {
    ...
    const orgCount = {};
    nycOpportunities.forEach(opp => {
      orgCount[opp.organization] = (orgCount[opp.organization] || 0) + 1;
    });
