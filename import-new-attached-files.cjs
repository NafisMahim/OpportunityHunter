...
// NEW files from latest upload
const NEW_FILES = [
  'free highschool programs + competitions - competitions_1752129277752.csv',
  'free highschool programs + competitions - free programs_1752129277754.csv',
  'Master List of Extracurriculars.xlsx - Competition_1752129277754.csv',
  'Master List of Extracurriculars.xlsx - Internship & Research _1752129277755.csv',
  'Master List of Extracurriculars.xlsx - Online courses_1752129277755.csv',
  'Master List of Extracurriculars.xlsx - Pre-college_1752129277755.csv',
  'Master List of Extracurriculars.xlsx - Volunteering_1752129277755.csv',
  'School Made Easy Opportunities  - Creative Arts Opportunities_1752129277756.csv',
  'School Made Easy Opportunities  - Medical Opportunities_1752129277756.csv',
  'School Made Easy Opportunities  - STEM Opportunities_1752129277756.csv',
  'School Made Easy Opportunities  - Voluneer Opportunities_1752129277756.csv',
  'STEM Programs_Internships - Other Programs_1752129277756.csv',
  'STEM Programs_Internships - Summer Internships_1752129277756.csv',
  'STEM Programs_Internships - Summer Programs_1752129277756.csv'
];
    ...
    for (const row of records) {
      try {
        ...
        const description = cleanText(
          ...
          row['Program Description'] || row['Brief Description'] || ''
        );
        
        const url = (
          row.URL || row.url || row.Website || row.Link || row.website || 
          row['Application URL'] || row['Program URL'] || row['Website / info'] || ''
        ).trim();
        ...
        if (!finalDescription || finalDescription.length < 10) {
          finalDescription = `${type} opportunity${organization ? ' from ' + organization : ''}. ${categories.length ? 'Focus areas: ' + categories.join(', ') + '.' : ''} Visit the website for complete details and application requirements.`;
        }
        ...
        `, [
          ...
          `New Import: ${path.basename(filePath)}`,
          categories,
          requirements
        ]);
...
async function processAllNewFiles() {
  try {
    await client.connect();
    console.log('🚀 Processing ALL new attached files...\n');
...
processAllNewFiles().catch(console.error);